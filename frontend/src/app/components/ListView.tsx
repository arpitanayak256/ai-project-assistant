...
export default function ListView({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'TODO': return 'bg-zinc-800 text-zinc-400 border border-zinc-700/50';
      ...
      default: return 'bg-zinc-950 text-zinc-500';
    }
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center bg-zinc-900/30 border border-zinc-800/80 p-4 rounded-2xl">
        <span className="text-xs text-zinc-400 font-semibold uppercase tracking-wide">Structured Epic List</span>
        <button
      ...
      {isAddingEpic && (
        <div className="bg-zinc-900/50 border border-zinc-800 p-5 rounded-2xl animate-slide-down">
          <h3 className="text-xs font-bold text-zinc-200 mb-3 uppercase tracking-wide">Create Project Epic</h3>
          <form onSubmit={handleCreateEpic} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-zinc-400 uppercase">Epic Name</label>
              <input
                type="text"
      ...
      <div className="space-y-3">
        {projectEpics.map((epic) => {
          ...
          return (
            <div key={epic.id} className="bg-zinc-900/30 border border-zinc-850 rounded-2xl overflow-hidden">
              {/* Epic Summary Header */}
              <div
                onClick={() => toggleEpic(epic.id)}
                className="flex items-center justify-between p-4 cursor-pointer hover:bg-zinc-900/40 select-none border-b border-zinc-850/50"
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-xs font-bold text-zinc-200 line-clamp-1">{epic.title}</h3>
                    <p className="text-[10px] text-zinc-500 line-clamp-1 mt-0.5">{epic.description}</p>
                  </div>
                </div>

                <div className="flex items-center gap-4 text-[10px] text-zinc-400 font-medium shrink-0">
              ...
              {isExpanded && (
                <div className="p-4 bg-zinc-950/30 space-y-3 animate-fade-in">
                          ...
                          epicTasks.map((task) => {
                            ...
                            return (
                              <tr
                                ...
                                className="group hover:bg-zinc-900/30 cursor-pointer"
                              >
                                <td className="py-3 font-semibold text-zinc-300 group-hover:text-indigo-400 transition-colors pr-2">
                                  {task.title}
                                </td>
                                <td className="py-3">
                                ...
                                <td className="py-3">
                                  {assignee ? (
                                    <div className="flex items-center gap-1.5 min-w-0">
                                      <img src={assignee.avatarUrl} className="w-5 h-5 rounded-full border border-zinc-800 shrink-0" />
                                      <span className="truncate text-zinc-400">{assignee.name.split(' ')[0]}</span>
                                    </div>
                                  ) : (
                    ...
                    {addingTaskEpicId === epic.id ? (
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-1.5 text-xs text-zinc-300 focus:outline-none focus:border-indigo-500"
                        />
                        <button
                        ...
                        <button
                          ...
                          className="border border-zinc-800 text-zinc-400 text-xs px-3 py-1.5 rounded-xl hover:text-zinc-200"
                        >
                          Cancel
                        </button>
                      </div>
        ...
        {unassignedTasks.length > 0 && (
          <div className="bg-zinc-900/30 border border-zinc-850 rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-zinc-850/50">
              <div className="flex items-center gap-3">
                <div className="p-1.5 bg-zinc-800 rounded-lg text-zinc-400">
                  <ShieldAlert className="w-4 h-4" />
                </div>
                <div>
                  <h3 className="text-xs font-bold text-zinc-200">Unassigned Tasks</h3>
                  <p className="text-[10px] text-zinc-500">Tasks not linked to any specific Epic</p>
                </div>
              </div>
            </div>
                        ...
                        <td className="py-3">
                          {assignee ? (
                            <div className="flex items-center gap-1.5">
                              <img src={assignee.avatarUrl} className="w-5 h-5 rounded-full border border-zinc-800" />
                              <span className="truncate text-zinc-400">{assignee.name.split(' ')[0]}</span>
                            </div>
                          ) : (
