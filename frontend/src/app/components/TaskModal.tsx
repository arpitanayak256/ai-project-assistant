        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-5 pr-2 custom-scrollbar">
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              className="w-full bg-transparent border-0 border-b border-transparent hover:border-zinc-200 dark:border-zinc-800 focus:border-indigo-500 text-sm font-bold text-zinc-900 dark:text-zinc-100 py-1.5 px-0 focus:outline-none"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-zinc-500 dark:text-zinc-500 uppercase">Status</label>
              <select
                value={status}
                onChange={(e) => { setStatus(e.target.value as any); onUpdateTask({ ...task, status: e.target.value as any }); }}
                className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl px-3 py-2 text-xs text-zinc-700 dark:text-zinc-300 focus:outline-none"
              >
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-zinc-500 dark:text-zinc-500 uppercase">Priority</label>
              <select
                value={priority}
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold text-zinc-500 dark:text-zinc-500 uppercase">Assignee</label>
              <select
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl py-2 pl-9 pr-4 text-xs text-zinc-700 dark:text-zinc-300 focus:outline-none"
                />
              </div>
            </div>
          </div>
          ...
          {/* Description */}
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold text-zinc-500 dark:text-zinc-500 uppercase">Detailed Description</label>
            <textarea
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              onBlur={handleSaveChanges}
              placeholder="Add detailed task requirements..."
              className="w-full h-24 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-3 text-xs text-zinc-700 dark:text-zinc-300 placeholder-zinc-650 focus:outline-none focus:border-indigo-500 resize-none leading-relaxed"
            />
          </div>
          ...
          <div className="space-y-3 pt-2">
            <div className="flex justify-between items-center">
              <label className="text-[10px] font-bold text-zinc-500 dark:text-zinc-500 uppercase">Subtasks Checklist ({completedSubtasks}/{taskSubtasks.length})</label>
              <button
                type="button"
            ...
            <div className="space-y-2 max-h-40 overflow-y-auto">
              {taskSubtasks.map((sub) => (
                <div key={sub.id} className="flex items-center justify-between gap-2 p-2 rounded-lg bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-850 hover:bg-zinc-50 dark:bg-zinc-900 transition-colors">
                  <div className="flex items-center gap-2 min-w-0">
                    <input
                      type="checkbox"
                      checked={sub.completed}
                      onChange={(e) => onToggleSubtask(sub.id, e.target.checked)}
                      className="w-3.5 h-3.5 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 text-indigo-650 focus:ring-indigo-500 rounded cursor-pointer"
                    />
                    <span className={`text-[11px] truncate leading-normal ${sub.completed ? 'line-through text-zinc-550' : 'text-zinc-700 dark:text-zinc-300'}`}>
                      {sub.title}
                    </span>
                  </div>
                  <button
                    onClick={() => onDeleteSubtask(sub.id)}
                    className="p-1 hover:bg-zinc-100 dark:bg-zinc-800/80 text-zinc-500 dark:text-zinc-500 hover:text-red-400 rounded shrink-0"
                  >
            ...
            <form onSubmit={handleAddSubtask} className="flex gap-2">
              <input
                ...
                className="flex-1 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-850 rounded-xl px-3 py-1.5 text-xs text-zinc-700 dark:text-zinc-300 focus:outline-none"
              />
              <button
                type="submit"
          ...
          {task.aiExplanation && (
            <div className="bg-indigo-950/20 border border-indigo-900/30 p-4 rounded-xl space-y-2">
              ...
              <p className="text-[11px] text-zinc-700 dark:text-zinc-300 leading-relaxed font-medium">{task.aiExplanation}</p>
            </div>
          )}
        </div>

        {/* Footer */}
        <div className="pt-4 border-t border-zinc-200 dark:border-zinc-850 flex justify-end shrink-0">
          <button
            onClick={onClose}
